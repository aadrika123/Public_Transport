...
if (!function_exists("responseMsgs")) {
    function responseMsgs($status, $msg, $data, $apiId = null, $version = null, $queryRunTime = null, $action = null, $deviceId = null,$datetransfer=false)
    {
        return response()->json([
            ...
            'meta-data' => [
                'apiId' => $apiId,
                'version' => $version,
                'responsetime' => responseTime()??$queryRunTime,
                'epoch' => Carbon::now()->format('Y-m-d H:i:m'),
                'action' => $action,
                'deviceId' => $deviceId
            ],
            'data' => $datetransfer?remove_null($data,false,[],$datetransfer):$data,
        ]);
    }
}
...
if (!function_exists("remove_null")) {
    function remove_null($data, $encrypt = false, array $key = ["id"],$dateChangeFormat=false)
    {
        $collection = collect($data)->map(function ($name, $index) use ($encrypt, $key,$dateChangeFormat) {
            if (is_object($name) || is_array($name)) {
                return remove_null($name, $encrypt, $key,$dateChangeFormat);
            } 
            else 
            {
                $pattern1 = "/created_at/i";
                $pattern2 = "/updated_at/i";
                $pattern3 = "/deleted_at/i";
                $pattern4 = "/date/i";
                if($dateChangeFormat && (preg_match($pattern1,$index) || preg_match($pattern2,$index) || preg_match($pattern3,$index) || preg_match($pattern4,$index) ))
                {                    
                    $name = $name?(strlen(explode("-",$name)[2]??"")>3?Carbon::parse($name)->format('d-m-Y h:i A'): Carbon::parse($name)->format('d-m-Y')):$name;
                }
...
if(!function_exists("paginater")){
    function paginater($orm,$request)
    {
